...
- 默认情况下，`qwn machine run --auto-start -b --on-conflict replace` 会拉起当前全部可见且健康的 GPU 后端
- 如果你需要精确控制 GPU 布局、模型或量化配置，优先手动执行 `qwn compose up ...`；`--auto-start` 更适合默认单模型部署
- 如果已经有其他 QWN 后端在运行，`--compose-gpus` 只影响 auto-start 新拉起的后端；这时应先手动执行 `qwn compose up --gpu-layout uniform-awq -g "0,1"`，再运行 `qwn machine run -b --on-conflict replace`
...
如果你想单独按步骤跑 QWN 的部署、machine、健康检查和 benchmark，可直接使用仓库内分步脚本：
...
- 默认使用当前 VM 中全部可见 GPU
- 具体 staged workflows 见 `runs/qwns/README.md`

## 联合恢复脚本

如果你要把本机 TEI `28800` 和 QWN `27800` 一起按当前全部可见 GPU 重启，并补完 live 验证，可直接执行仓库内脚本：

```bash
bash runs/recovery/restart_tei_qwn.sh
```

若你还想顺手跑一轮更高负载的共存压测，可额外打开 benchmark：

```bash
TEI_BENCH_SAMPLES=60000 QWN_BENCH_SAMPLES=80 bash runs/recovery/restart_tei_qwn.sh
```

- 默认会先等待 direct backend `/health`，再启动 machine；这比直接盯 Docker healthcheck 更稳，因为后者可能晚于真实 HTTP 健康收敛
- 默认 TEI 校验是 3 轮 `240` 输入的 `/lsh` 请求；默认 QWN 校验包含 `/v1/models`、真实 chat completion，以及轻量 `qwn benchmark`
- 恢复日志写入 `runs/recovery/results/`；若开启 benchmark，结果会分别写入 `runs/teis/results/` 与 `runs/qwns/results/`
