"""Shared infrastructure for multi-machine QSR clients."""
...
class MachineState:
    endpoint: str
    client: QSRClient = field(repr=False)
    async_client: AsyncQSRClient = field(default=None, repr=False)
    healthy: bool = False
    healthy_instances: int = 0
    total_instances: int = 0
    _active_requests: int = 0
    _max_concurrent: int = MAX_NUM_SEQS
...
class BatchRequestOutcome:
    response: ChatResponse | StreamChatResult | TranscriptionResponse | None = None
...
class _QSRClientsPipeline:
    ...
    def run_pipeline_outcomes(
        ...
        request_fn: Callable,
        action_name: str = "request_batch",
        close_clients: bool = True,
    ) -> list[BatchRequestOutcome]:
    ...
    async def _run_pipeline_async(
        ...
        results: list[BatchRequestOutcome | None] = [None] * total
        ...
        try:
            while next_index < total or tasks:
                ...
                for finished in done:
                    ...
                    if error is None and response is not None:
                        ...
                        results[index] = BatchRequestOutcome(
                            response=response,
                        ...
                        results[index] = BatchRequestOutcome(
                            error=error, latency_sec=latency_sec
                        )
                    completed += 1
                    if self.on_progress:
            ...
            final_results = [
                (
                    ...
                    else BatchRequestOutcome(
                        error=RuntimeError("missing pipeline result")
                    )
                )
...
class _QSRClientsBase(ABC):
    def __init__(self, endpoints: list[str]):
        ...
        self._pipeline: Optional[_QSRClientsPipeline] = None
        self._rr_index = 0
    ...
    def chat_batch_outcomes(
        ...
        if len(requests) == 1:
            ...
            try:
                ...
                return [
                    BatchRequestOutcome(
                        error=exc, latency_sec=time.perf_counter() - started_at
                    )
                ]
        ...
        if measure_ttft:
            ...
            async def request_fn(
                ...
                return await machine.async_client.stream_chat(
                    ...
                    max_tokens=request_dict.get("max_tokens", DEFAULT_MAX_TOKENS),
                    temperature=request_dict.get("temperature", 0.0),
    ...
    def transcribe(
        ...
        machine = healthy[self._rr_index % len(healthy)]
        self._rr_index += 1
        return machine.client.transcribe(
            audio=audio,
            model=model,
            language=language,
            prompt=prompt,
    ...
    def transcribe_batch_outcomes(
        ...
        if len(requests) == 1:
            ...
            started_at = time.perf_counter()
            try:
                response = machine.client.transcribe(
                    audio=request["audio"],
                    model=request.get("model", model),
                    language=request.get("language", language),
                    prompt=request.get("prompt", prompt),
                    response_format=request.get("response_format", response_format),
                    temperature=request.get("temperature", temperature),
                    timestamp_granularities=request.get(
                ...
                latency_sec = time.perf_counter() - started_at
                setattr(response, "_latency_sec", latency_sec)
                return [BatchRequestOutcome(response=response, latency_sec=latency_sec)]
            except Exception as exc:
                return [
                    BatchRequestOutcome(
                        error=exc, latency_sec=time.perf_counter() - started_at
        ...
        async def request_fn(
            ...
            return await machine.async_client.transcribe(
                audio=request_dict["audio"],
                model=request_dict.get("model", ""),
                language=request_dict.get("language"),
                prompt=request_dict.get("prompt"),
                response_format=request_dict.get("response_format", "json"),
                temperature=request_dict.get("temperature"),
                timestamp_granularities=request_dict.get("timestamp_granularities"),
            )
